 ...
 */
 class ListsSuite {
  ...
  @Test def `max of one number`: Unit = {
    assertEquals(69, max(List(69)))
  }

  @Test def `max of negative numbers`: Unit = {
    assertEquals(-69, max(List(-69, -420)))
  }

  @Test def `max of repeating numbers`: Unit = {
    assertEquals(69, max(List(69, 69, 69, -420)))
  }

  @Test def `max of no numbers`: Unit = {
    try {
      max(List())
      Assert.fail("No exception has been thrown.")
    } catch {
