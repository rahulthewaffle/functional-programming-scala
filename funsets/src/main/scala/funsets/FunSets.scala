...
trait FunSets extends FunSetsInterface {
   ...
   */
  def diff(s: FunSet, t: FunSet): FunSet = elem => if (s(elem)) !t(elem) else false
  ...
  def map(s: FunSet, f: Int => Int): FunSet = {
    @tailrec
    def iter(a: Int, cumSet: FunSet): FunSet = {
      if(a > bound) cumSet
      else if (s(a)) iter(a+1, union(cumSet, x => x == f(a)))
      else iter(a+1, cumSet)
    }
    iter(-bound, x => false)
  }
